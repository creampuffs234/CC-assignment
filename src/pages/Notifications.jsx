...
export default function Notifications() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // 🔍 Detect if logged-in user is a shelter admin
        ...
        {notifications.length === 0 ? (
          ...
          <div className="space-y-4">
            {notifications.map((n) => (
              <div
                key={n.id}
                className={`p-5 rounded-xl shadow-lg border backdrop-blur-md transition
                  ${
                    n.read
                      ? "bg-white/10 border-white/10"
                      : "bg-indigo-600/30 border-indigo-600 shadow-indigo-500/30"
                  }`}
              >
